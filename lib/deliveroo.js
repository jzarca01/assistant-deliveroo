...
class AssistantDeliveroo {
  ...
  async trackOrder(accessToken, userId) {
    try {
      const login = await deliveroo.setAccessToken(userId, accessToken);
      const history = await deliveroo.getHistory(userId);
      const lastOrder = history.orders
        ? history.orders[0].consumer_status
        : null;
      if (lastOrder.current_step !== this.current_step.current_step) {
      ...
      if (this.current_step.code === "COMPLETE") {
        ...
        setTimeout(async () => await this.trackOrder(accessToken, userId), 120000);
      }
    } catch (err) {
      if (this.plugins.notifier)
        this.plugins.notifier.action(
          `Un problème est survenu avec le traqueur`
        );
      console.log("err", err);
    }
  }
